...
One major theme you need to focus when learning every new language is `string`. Maybe you had been learning in some steps: variable, assignment, string, operators... String operation is definitely one important factor in programming.
...
In string operation we usually need to process some common problems such as: find, replacing,... Most of them need to find the substring and implement some operations. Maybe you did like this in the very beginning of your learning path.
...
`Regex` is a efficient tool to solve that problem; but it come with the price. It is really hard to read and understand (but not hard to learn). First try to read the example below.
